...
def save_state(database_name):
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS all_room_data "
                 ...
                 "offices TEXT, living_spaces TEXT, rooms TEXT,"
                 " office_details TEXT, living_space_details TEXT,"
                 " all_rooms_details TEXT, fellows TEXT, staff TEXT,"
                 " persons TEXT, unallocated_staff TEXT,"
                 " unallocated_fellows TEXT, allocated_rooms TEXT,"
                 " allocated_persons TEXT)")
    ...
    conn.execute("INSERT INTO all_room_data VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? );",
                 (offices, living_spaces, rooms, office_details,
                  living_space_details, all_rooms_details, fellows, staff,
                  persons, unallocated_staff,
                  unallocated_fellows, allocated_rooms, allocated_persons))
...
def load_state(database_name):
    ...
    conn = db_connect.cursor()
    conn.execute(
        "SELECT * FROM all_room_data WHERE dataID = (SELECT MAX(dataID) FROM all_room_data)")
    data = conn.fetchone()
    ...
    for person in content:
        ...
        if length == 4:
            first_name, last_name, role, accomodation_status = person.split(
                ' ')
            Amity.add_person(first_name, last_name, role, accomodation_status)
        if length == 3:
            first_name, last_name, role = person.split(' ')
            Amity.add_person(first_name, last_name, role, "N")
