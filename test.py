...
class MyTests(unittest.TestCase):
    ...
    def test_it_creates_room(self):
        self.assertEqual(Amity.create_room("aseey", "office"),
                         "ROOM SUCCESSFULLY CREATED IN AMITY")
    ...
    def test_does_not_allocate_staff_room_if_no_room(self):
        self.assertEqual(Amity.allocate_staff_room(
            "didah", "aseey", "STAFF", "N"), "NO ROOMS AVAILABLE!!")

    def test_does_not_allocate_fellow_room_if_no_room(self):
        self.assertEqual(Amity.allocate_fellow_with_no_accomodation(
            "didah", "aseey", "FELLOW", "N"), "NO ROOMS AVAILABLE!!")

    def test_does_not_allocate_fellow_room_if_no_room(self):
        self.assertEqual(Amity.allocate_fellow_with_accomodation(
            "didah", "aseey", "FELLOW", "N"), "NO ROOMS AVAILABLE!!")

    def test_it_allocates_office(self):
        Amity.create_room("narnia", "office")
        self.assertEqual(Amity.allocate_staff_room(
            "didah", "aseey", "STAFF", "N"), 'ROOM SUCCESSFULLY ASSIGNED!!')

    def test_it_allocates_fellow_office(self):
        Amity.create_room("narnia", "office")
        self.assertEqual(Amity.allocate_fellow_with_no_accomodation(
            "didah", "aseey", "FELLOW", "N"), 'ROOM SUCCESSFULLY ASSIGNED!!')

    def test_it_allocates_fellow_living_space(self):
        Amity.create_room("narnia", "office")
        Amity.create_room("dojo", "living space")
        self.assertEqual(Amity.allocate_fellow_with_accomodation(
            "didah", "aseey", "FELLOW", "Y"), 'ROOM SUCCESSFULLY ASSIGNED!!')
    ...
    def test_does_not_duplicate_room_names(self):
        ...
        self.assertEqual(Amity.create_room(
            "dojo", "living space"), "ROOM EXISTS!")

    def test_only_accepts_office_or_living_space(self):
        self.assertEqual(Amity.create_room(
            "java", "watchtower"), "INVALID ROOM TYPE INPUT!!")
    ...
    def test_printer_fellow_helper(self):
        self.assertEqual(amity.printer_fellow_N(
            'full_name', 'role', 'office'), None)

    def test_living_space_reallocator_printer_helper(self):
        self.assertEqual(amity.living_space_reallocator_printer(
            'full_name', 'role', 'office', 'living_space'), None)
